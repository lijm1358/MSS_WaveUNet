...
def main(args):
    ...
    if not args.no_sep_segment:
        ...
        separate_segment(
            target_dir_numpy_train, target_dir_split_train, sep_length_train
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process MUSDB18(https://sigsep.github.io/datasets/musdb.html) audio dataset into same length numpy array segments.",
    ...
    parser.add_argument(
        "base_dir",
        type=str,
        help='The directory where MUSDB18 dataset is stored. Should contain the directory "train" and "test".',
    )
    parser.add_argument(
        "target_dir",
        type=str,
        help="The directory where the processed dataset will be stored",
    )
    parser.add_argument(
        "--sep_length_train",
        type=int,
        default=16384,
        help="length of the separated segments of train dataset. Default value is 16384 along with the baseline model of original paper.",
    )
    parser.add_argument(
        "--sep_length_test",
        type=int,
        default=22050,
        help="length of the separated segments of test dataset",
    )
    parser.add_argument(
        "--no_sep_stem",
        default=False,
        action="store_true",
        help="ignore the stream separation step(optional)",
    )
    parser.add_argument(
        "--no_numpy",
        default=False,
        action="store_true",
        help="ignore the numpy array conversion step(optional)",
    )
    parser.add_argument(
        "--no_augment",
        default=False,
        action="store_true",
        help="ignore the data augmentation step(optional)",
    )
    parser.add_argument(
        "--no_sep_segment",
        default=False,
        action="store_true",
        help="ignore the segment separation step(optional)",
    )
